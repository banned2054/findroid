                             ...
                             ): AlertDialog
{
    val locations = context.getExternalFilesDirs(null).mapNotNull {
        val locationStringRes =
            if (Environment.isExternalStorageRemovable(it)) CoreR.string.external else CoreR.string.internal
        val stat = StatFs(it.path)
        context.getString(
            CoreR.string.storage_name,
            context.getString(locationStringRes),
            stat.availableBytes.div(1000000)
                         )
    }.toTypedArray()
